function [input,path]=fuzzy_MPC_path_planning(state,obstacle,destination,system_model,NUMBER_OF_INPUTS,...
        horizon,terminal_horizon,Space_size,terminal_set,target_velocity,max_linear_velocity,min_disttance,safe_distance,weigths)
...
full_cost_function = @(trajectory) compute_cost(trajectory,full_fuzzy_stage_cost,fuzzy_terminal_cost);

lb=-ones(horizon*NUMBER_OF_INPUTS,1)*max_linear_velocity;
ub=-lb;

nonlocon_function = @(trajectory) check_if_in_terminal_set(trajectory,state,terminal_set,horizon,NUMBER_OF_INPUTS,system_model);
